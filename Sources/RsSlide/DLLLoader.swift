...
final class DLLLoader {
    ...
    init(name: String, folder: String) {
        ...
        if let url = Bundle.main.url(
            forResource: name, withExtension: ".dll",
            subdirectory: "Ruslan_Ruslan.resources/CloudExec/lib/\(folder)")
        {  // For Ruslan project
            path = url.filePath
        } else if let url = Bundle.main.url(
            forResource: name, withExtension: ".dll", subdirectory: "\(folder)")
        {  // For RsSlide project
            path = url.filePath
            ...
            if bufSize > 0 {
                let rjSlideUrl = URL(
                    filePath: String(decoding: Array(buf[..<Int(bufSize)]), as: UTF16.self))
                if let folder = rjSlideUrl.reachableSibling(named: "\(folder)/"),
                    let url = folder.reachableChild(named: "\(name).dll")
                {
                    path = url.filePath
                }
            }
        ...
        dll = LoadLibraryExW(
            path.wideString, nil,
            DWORD(LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_DEFAULT_DIRS))  // Load other dependencies from the same DLL folder.
        if dll != nil {
